...
export default function Navbar({ simplified = false }) {
  ...
  // Check if we're on a footer page (not landing page)
  const isFooterPage = location.pathname !== '/';

  // Use simplified navbar on footer pages
  const showSimplified = simplified || isFooterPage;
    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          {showSimplified ? (
            /* Simplified Menu - Only Get Started */
            ...
            <>
              ...
              <div className="hidden md:block">
                <div className="ml-10 flex items-center space-x-8">
                  ...
                  <a href="#tech" className="text-gray-600 dark:text-gray-300 hover:text-primary-600 dark:hover:text-primary-400 font-medium transition-colors">Tech Stack</a>
                  <Link to="/login">
                    <Button>Get Started</Button>
                  </Link>
                </div>
